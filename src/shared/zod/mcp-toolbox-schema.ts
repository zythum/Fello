...
export const screenshotRequestSchema = z.object({
  output: z
    ...
    .optional()
    .describe(
      "Output file path for the screenshot. If omitted, saves as 'screenshot.{format}' in project root.",
    ),
  format: z
    .enum(["png", "jpeg", "webp"])
    .default("png")
    .describe("Image format for the screenshot. Default: png."),
  screen: z
    .number()
    .int()
    .optional()
    .describe(
      "Display/screen number to capture on a multi-monitor setup (0-based index). If omitted, captures all displays combined.",
    ),
  width: z
    .number()
    .int()
    ...
    .optional()
    .describe(
      "Target width in pixels. If set, the screenshot will be resized to this width (maintaining aspect ratio). Useful on Retina/HiDPI displays to reduce image size.",
    ),
});

export const screenshotRespondSchema = z.object({
  result: z.object({
    output: z.string(),
    screenWidth: z
      .number()
      .optional()
      .describe("Original screen width in pixels before any resizing."),
    screenHeight: z
      .number()
      .optional()
      .describe("Original screen height in pixels before any resizing."),
    metadata: imageMetadataRespondSchema.shape.result,
  }),
});

export type ScreenshotRequest = z.infer<typeof screenshotRequestSchema>;
export type ScreenshotRespond = z.infer<typeof screenshotRespondSchema>;

// ── List Displays ────────────────────────────────────────────────────

export const listDisplaysRequestSchema = z.object({});

export const listDisplaysRespondSchema = z.object({
  result: z.array(
    z.object({
      id: z
        .number()
        .describe("0-based display index, used as screen parameter in screenshot tool."),
      name: z.string().describe("Display name."),
      primary: z.boolean().describe("Whether this is the primary/main display."),
    }),
  ),
});

export type ListDisplaysRequest = z.infer<typeof listDisplaysRequestSchema>;
export type ListDisplaysRespond = z.infer<typeof listDisplaysRespondSchema>;
