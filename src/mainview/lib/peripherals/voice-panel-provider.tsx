 ...
 * - **松手后自动发送**：复核态在「取消 / 发送」旁边显示 5 秒倒数（常量与倒计时实现都在
 *   `chat-textarea.tsx` 的 `VOICE_PANEL_AUTO_SEND_SECONDS`），归零即走与点「发送」**完全
 *   相同**的路径提交，因此来源仍是 `voice`、调用方的打断与焦点收尾照旧；窗口期内点「取消」
 *   或按遥控器「返回」键（Escape）即可反悔，再次按住语音键则本次倒数作废、继续往后追加，
 *   松手后重新从头倒数；文本为空（直接收掉，不进复核态）或有错误时不倒数。
