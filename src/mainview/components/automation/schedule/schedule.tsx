...
import { LoaderCircle, ArrowLeft, Play, Settings2, CheckCircle2, XCircle, Trash2 } from "lucide-react";
...
export function Schedule() {
    ...
    const h = (p: any) => {
      if (p.scheduleId === scheduleId) {
        setTasks((prev) => {
          ...
          if (idx >= 0) { const n = [...prev]; n[idx] = p.task; return n; }
          return [p.task, ...prev];
        });
      }
    };
  ...
  const handleTrigger = async () => {
    if (!scheduleId) return;
    try { await request.triggerSchedule({ scheduleId }); }
    catch { /* ignore */ }
  };
  ...
  const getTaskIcon = (task: Task) => {
    switch (task.status) {
      case "running": return <LoaderCircle className="size-3.5 animate-spin text-blue-500" />;
  ...
  if (loading) {
    return (
      <main className="flex min-w-0 flex-1 flex-col relative overflow-hidden">
        <div className="h-12 shrink-0 border-b border-border flex items-center px-6" style={{ WebkitAppRegion: "drag" }}>
          <h1 className="text-sm font-medium">—</h1>
        </div>
        <div className="flex flex-1 items-center justify-center"><LoaderCircle className="size-6 animate-spin text-muted-foreground" /></div>
      </main>
    );
  }
  ...
  if (!schedule) {
    return (
      <main className="flex min-w-0 flex-1 flex-col items-center justify-center relative overflow-hidden gap-3">
        ...
        <p className="text-sm font-normal text-muted-foreground">{t("automation.scheduleNotFound", "Schedule not found")}</p>
        <Button variant="outline" size="sm" className="h-7 text-xs" onClick={() => navigate("/automation")}>
          <ArrowLeft className="size-3" />{t("automation.back", "Back")}
        </Button>
      </main>
    );
  }
  ...
  return (
    <main className="flex min-w-0 flex-1 flex-col relative overflow-hidden">
      {/* Header */}
      <div className="h-12 shrink-0 border-b border-border flex items-center gap-1 px-2" style={{ WebkitAppRegion: "drag" }}>
        <div style={{ WebkitAppRegion: "no-drag" }}>
          <Button variant="ghost" size="icon" className="size-7 shrink-0" onClick={() => navigate("/automation")}>
        ...
        <div className="flex items-center gap-2" style={{ WebkitAppRegion: "no-drag" as any }}>
          <Button variant="ghost" size="icon" className="size-7" onClick={() => setEditDialogOpen(true)} title={t("automation.editSettings", "Edit settings")}>
            <Settings2 className="size-3.5" />
          </Button>
          <Button variant="default" size="sm" className="h-7 text-xs shrink-0" onClick={handleTrigger}>
      ...
      {tasks.length === 0 ? (
        <div className="flex flex-1 flex-col items-center justify-center gap-2 text-center px-6">
          <p className="text-xs text-muted-foreground">
            {t("automation.noTasksDesc", "No runs yet. Click \"Trigger now\" to start the first run.")}
          </p>
        </div>
      ) : (
      <ResizablePanelGroup className="flex-1 min-h-0">
        {/* Left: Task list */}
        <ResizablePanel id="taskList" groupResizeBehavior="preserve-pixel-size" defaultSize={200} minSize={180} maxSize={400}>
          <div className="h-full flex flex-col">
            <ScrollArea className="flex-1">
              <div>
                {[...tasks].sort((a, b) => b.startedAt - a.startedAt).map((task) =>
                  {
