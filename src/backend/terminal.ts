...
export function initTerminal(emitter: typeof sendEvent) {
  sendEvent = emitter;
}
...
export async function registerClient({ clientId }: { clientId: string }) {
...
export async function writeTerminal({ terminalId, data }: { terminalId: string; data: string }) {
...
export async function killTerminalsByClient({ clientId }: { clientId: string }) {
...
export async function killTerminal({ terminalId }: { terminalId: string }) {
  const terminal = terminals.get(terminalId);
  if (!terminal) return {};
  terminal.kill();
  terminals.delete(terminalId);
...
export async function getAgentTerminalOutput({
  sessionId,
  terminalId,
}: {
  sessionId: string;
